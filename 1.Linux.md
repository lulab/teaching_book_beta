...
```
wget http://www.ncrnalab.org/lulab/public/Sa*.gtf.gz

```

然后

将Saccharomyces_cerevisiae.R64-1-1.77.gtf.gz拷贝到自己创建的目录下，并解压缩该文件

```bash
cp ./Sa*.gtf.gz my_folder/
cd my_folder
gunzip *.gtf.gz
```

注意：命令cp、文件“./Sa*.gtf.gz”、目录“my_folder/”之间均存在空格。



>** Note：**
> 
> **该文件在"虚拟机"中已经下载好，位置在/home/cs/Bioinfo_Lab/0.Linux/目录下，文件名为Sa*.gtf.gz**
...
```bash
cd my_folder
cp /home/cs/Bioinfo_Lab/0.Linux/Sa*.gtf.gz  .
gunzip *.gtf.gz
```
>**Note：**
>
...
> 
> **tab键可以实现命令名及文件名的自动补全。**
> 
> **使用tab键、"."和".."可以减少输入命令时的工作量。**
...
```
#!/bin/bash   
grep exon Sa*.gtf | awk '{print $5-$4+1}' | sort -n | tail -3
```
